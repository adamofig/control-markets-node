...
describe('InboxTaskAutomationService', () => {
  it('opens a task thread and publishes an idempotent automatic message for a human assignee', async () => {
    ...
    const messages = { send: jest.fn().mockResolvedValue({ message: { id: 'message-1' }, agentResponseExpected: false }) };
    const service = new InboxTaskAutomationService(tasks as any, identities as any, conversations as any, messages as any);

    await service.onTaskChanged({ id: 'task-1' });

    expect(conversations.getOrCreateTask).toHaveBeenCalledWith('org-1', expect.objectContaining({ participantId: 'system:zazu', refId: 'zazu' }), 'task-1', 'Tarea · Preparar campaña', [assignedUser]);
  ...
  it('does not create Inbox threads for non-human assignments', async () => {
    ...
    const service = new InboxTaskAutomationService(tasks as any, {} as any, conversations as any, {} as any);
