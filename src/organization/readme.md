...
Organization is just a scaffold to create a new entity in the database. Name Organization will be renamed to the name you want.
...
There are 3 ways to update a organization entity

#### POST /api/organization/:id - Full Update

- POST /api/organization/:id This is the only one with double functionality, if id is not present it will create a new entity if id is present it will update the existing entity with the new information.
...
#### PUT /api/organization/:id - Partial Update Root Level

- PUT /api/organization/:id This is the second way to update a organization entity, it will update the existing entity with the new information, but ignoring nulls.
...
#### PATCH /api/organization/:id - Partial Update Granular Level

- PATCH /api/organization/:id This is the third way to update a organization entity, and the most granular, every property will be flattened, backend will build mongo special structure so you have to build your object at this granular level. example { "video.captions.remotion.captions": captions.captions } = { video: { captions: { remotion: { captions: captions } } } }
